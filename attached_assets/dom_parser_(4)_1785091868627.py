...
# Meta shows a status badge ("Active" / "Inactive" / "Paused") on every
# ad card, often before the advertiser name in DOM order. Same rationale
# as skipping the Library ID / Started running text: it's boilerplate,
# not the name, so treating it as "the first short text node" silently
# returns the wrong value instead of the real advertiser name.
STATUS_BADGE_WORDS = {"active", "inactive", "paused"}

# UI chrome/boilerplate labels that have shown up as a mistaken
# advertiser_name at least once on this project. Not exhaustive by
# design -- new entries get added here as they're found, same as
# STATUS_BADGE_WORDS, but this set is now a backstop rather than the
# primary defense (see _guess_advertiser_name's Sponsored anchor).
_KNOWN_BAD_NAME_VALUES = {
    "platforms", "sponsored", "library id", "open drop-down",
    "see ad details", "learn more", "sign up", "shop now",
}
...
def _guess_advertiser_name(card):
    """
    Return the advertiser name from the card.

    Anchored on "Sponsored": every Meta ad card renders the advertiser
    name immediately followed by the "Sponsored" label, after a run of
    boilerplate (zero-width spaces, the Active/Inactive status badge,
    Library ID, Started running on, Platforms, Open Drop-down, See ad
    details). That boilerplate run has kept growing every time a new
    real card surfaced a new label in front of the name — a blacklist
    approach means playing whack-a-mole indefinitely. "Sponsored" itself
    is Meta's own disclosure text, immediately after the name, on every
    card (verified 29/29 against the real capture in page.html), so
    anchoring on it is stable in a way the blacklist never was.

    Falls back to the old first-short-text-node heuristic if "Sponsored"
    isn't present in the card at all, so cards from a differently laid
    out page (or the older synthetic fixtures) still degrade gracefully
    instead of returning None outright.
    """
    texts = [t.strip() for t in card.find_all(string=True)]
    texts_clean = ["".join(c for c in t if c not in _ZERO_WIDTH_CHARS).strip() for t in texts]

    for i, t_clean in enumerate(texts_clean):
        if t_clean == "Sponsored" and i > 0:
            name = texts_clean[i - 1]
            if name:
                return name
            break  # "Sponsored" found but nothing usable before it; fall through

    # Fallback: previous heuristic, for pages/fixtures without a
    # "Sponsored" marker at all.
    for t_clean in texts_clean:
        if not t_clean:
            continue
        if LIBRARY_ID_MARKER in t_clean or t_clean.startswith("Started running"):
            continue
        if t_clean.lower() in STATUS_BADGE_WORDS:
            continue
        if len(t_clean) < 60:
            return t_clean
    return None
...
def is_suspicious_advertiser_name(name):
    """
    True if `name` looks like a parsing miss rather than a real business
    name, so the caller can flag the record for review instead of
    letting a bad value flow silently into scoring.

    This exists because every advertiser_name bug hit on this project so
    far (zero-width space, "Active"/"Inactive" status badge, "Platforms"
    label) had the same shape: the parser confidently returned *some*
    string instead of admitting it wasn't sure. A name-guessing function
    can always be wrong again on the next Meta layout tweak — this is
    the safety net so a future miss gets caught automatically instead of
    requiring someone to manually diff raw JSON to notice.

    Deliberately conservative: only flags patterns we've actually seen
    or that are structurally implausible as a business name, not every
    short or unusual name (a real 2-letter brand shouldn't get flagged
    on length alone, so the length check is generous).
    """
    if not name:
        return True
    cleaned = "".join(c for c in name if c not in _ZERO_WIDTH_CHARS).strip()
    if not cleaned:
        return True
    if len(cleaned) < 2:
        return True
    if cleaned.lower() in STATUS_BADGE_WORDS:
        return True
    if cleaned.lower() in _KNOWN_BAD_NAME_VALUES:
        return True
    return False
