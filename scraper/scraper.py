...
from dom_parser import parse_ad_cards, is_suspicious_advertiser_name
...
def _parse_graphql_payload(payload: dict) -> list[ScrapedAd]:
    ...
    for edge in edges:
        ...
        for ad in collated:
            ...
            resolved_status = "ok" if unwrapped.get("parse_ok") else "needs_review"
            if resolved_status == "ok" and is_suspicious_advertiser_name(advertiser_name):
...
def _scrape_one_url(url: str, page, rate_limiter: RateLimiter, session: ScrapeSession) -> str:
    ...
    if not found_any:
        ...
        for r in dom_results:
            dom_status = r["status"]
            if dom_status == "ok" and is_suspicious_advertiser_name(r["advertiser_name"]):
                dom_status = "needs_review"
