...
def _find_outbound_link(card):
    """
    Return the best outbound link from an ad card.
    Preference order (confirmed against real Meta Ad Library HTML, 2026-07-23):
      1. A Meta shim link (l.facebook.com / lm.facebook.com) wrapping the
         real advertiser destination — this is what the CTA button uses.
      2. Any other http(s) link that isn't a Facebook-owned domain (direct
         advertiser domain, rare but present on some ad types).
    Direct facebook.com profile links are excluded — they're the page
    avatar/name link, not the ad's landing page, and they appear before the
    CTA link in the DOM so a naive first-match would pick the wrong one.
    """
    ...
    for a in card.find_all("a", href=True):
        ...
        try:
            ...
            continue
        if host in SHIM_HOSTS and shim_link is None:
            shim_link = a
        elif host not in SKIP_HOSTS and direct_link is None:
            direct_link = a
    return shim_link or direct_link
