...
def make_graphql_payload(ads):
    """
    Builds a payload matching the real confirmed field path (see
    scraper._parse_graphql_payload docstring, live capture 2026-07-25):
      data.ad_library_main.search_results_connection.edges[]
        .node.collated_results[]
        .{ad_archive_id, page_name, snapshot.link_url}

    NOTE: this used to build data.ad_library_main.results.edges[].node
    directly (no search_results_connection wrapper, no collated_results
    list) -- a shape that predates the field-path confirmation and no
    longer matches _parse_graphql_payload, which silently made every
    GraphQL-happy-path test below produce 0 results instead of the
    expected count. Fixed to match the parser it's meant to exercise.
    """
    edges = []
    for ad in ads:
        edges.append({"node": {
    ...
    return {"data": {"ad_library_main": {"search_results_connection": {"edges": edges}}}}
